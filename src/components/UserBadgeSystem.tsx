...
// Helper function to determine verification level from user data (matches your existing system)
export function getUserVerificationLevel(user: any): string {
  ...
  // Business account with both ID and business verification
  if (user.accountType === 'business' && user.verified && user.businessVerified) {
    return 'business-verified';
  }
  
  // Individual account with ID verification OR business account with only ID verification
  if (user.verified) {
    return 'id-verified';
  }
