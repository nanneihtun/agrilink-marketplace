// Script to create 12 users with different combinations
  ...
  for (const user of users) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Created user: ${user.name} (${user.user_type} - ${user.account_type} - ${user.verified ? 'verified' : 'unverified'})`);
      } else {
        const error = await response.text();
        console.log(`❌ Failed to create user ${user.name}:`, error);
      }
    } catch (error) {
